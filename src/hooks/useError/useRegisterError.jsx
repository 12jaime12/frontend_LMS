...
const useRegisterError = (res, setRegisterOk) => {
  ...
  if (
    res?.response?.data.includes(
      'duplicate key error collection: LegendaryMotorsport.users index: email_1 dup key: { email:'
    )
  ) {
    Swal.fire({
      icon: 'error',
  ...
  if (
    res?.response?.data.includes(
      'duplicate key error collection: LegendaryMotorsport.users index: movil_1 dup key: { movil:'
    )
  ) {
    Swal.fire({
      icon: 'error',
